rankhospital <- function(state, outcome,ranking) {
    ...
    ## getting the total number of hospitals in that state
    totalNum <- nrow(stateData)
    noNaNum <- length(stateData[,colNum][!is.na(stateData[,colNum])])
    ## test the ranking of the hospital 
    if(ranking == "best") {
        index <- 1
    }
