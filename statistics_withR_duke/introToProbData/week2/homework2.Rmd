...
sfo_feb_flights %>%
  summarise(mean_delay = mean(arr_delay), median_del = median(arr_delay), max_del = max(arr_delay), min_del = min(arr_delay))
```
...
originDelay <- sfo_feb_flights %>%
  group_by(carrier) %>%
    summarise(IQR(arr_delay))

max(originDelay$`IQR(arr_delay)`)
```
...
nycflights %>%
  group_by(month) %>%
  summarise(medDelay = median(dep_delay)) %>%
  arrange(desc(medDelay))
```
...
## note that the asked percentage is the percentage of flight whose departure is delayed
## but arrived on time in the flight whose departure is delayed. i.e.  \frac{dep_type = "delayed" ^ arr_type = "on time"}{dep_type = "delayed"} 
delayDepFlight <- nycflights %>%
  filter(dep_type == "delayed")
arrOn <- delayDepFlight %>%
  summarize(delayDepartArrOn = sum(arr_type == "on time")/n())
